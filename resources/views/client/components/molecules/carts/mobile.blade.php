@php $total = 0; $voucherValue = session('voucherValue'); @endphp
@if(session('cart'))
    ...
    @endif
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <label for="subtotal">Tổng tiền</label>
                    <p class="font-bold">$<span class="product-subtotal">{{$details['price'] * $details['quantity']}}</span></p>
                </div>
                <div class="col-12">
                    <label for="quantity">Số lượng : {{request()->routeIs('clientCheckout') ? 'x'.$details['quantity'] : ''}}</label>
                    @if(!request()->routeIs('clientCheckout'))
...
<div class="card mt-3 mb-4">
    <div class="card-body">
        <h5 class="font-secondary"><strong>Tổng tiền : <span class="cart-total">
            @if($voucherValue)
                <del class="text-danger">${{ $total }}</del> <!-- Giá trị ban đầu -->
                <span>${{ $total - ($total * ($voucherValue / 100)) }}</span> <!-- Giá trị sau khi áp dụng voucher -->
            @else
            ${{ $total }} <!-- Giá trị không áp dụng voucher -->
            @endif
        </span></strong></h5>
        @if(!request()->routeIs('clientCheckout'))
            <div class="d-flex justify-content-between mt-3 font-secondary">
                ...
                <a href="/checkout" class="btn btn-sm btn-primary">Thanh toán</a>
            </div>
        @endif
    </div>
</div>
