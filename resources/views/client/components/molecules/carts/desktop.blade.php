@php $voucherValue = session('voucherValue'); @endphp
    ...
    <tfoot>
        ...
        <tr>
            ...
            <td class="hidden-xs">
                <strong>
                    @if($voucherValue)
                        <del class="cart-total text-danger">${{ $total }}</del> <!-- Giá trị ban đầu -->
                        <span class="cart-total">${{ $total - ($total * ($voucherValue / 100)) }}</span> <!-- Giá trị sau khi áp dụng voucher -->
                    @else
                        <span class="cart-total">${{ $total }}</span> <!-- Giá trị không áp dụng voucher -->
                    @endif
                </strong>
            </td>
            @if(!request()->routeIs('clientCheckout'))
