...
class ClientController extends Controller
{
    ...
    public function checkoutSave(Request $request){
        ...
        if($validator->fails()){
            ...
            if(session('cart')){
                ...
                var_dump($total);
                var_dump(session('voucherValue'));
                if(!is_null(session('voucherValue'))){
                    $voucherValue = session('voucherValue');
                    $total = $total - ($total * ($voucherValue / 100));
                }
    ...
    public function checkVoucher(Request $request){
        ...
        if($validator->fails()){
            return redirect()->route('clientCheckout')->withErrors(['voucher'=>"Bạn chưa nhập mã giảm giá!"]);
        }else{
            $voucherCode = $request->voucher;
            $voucher = Voucher::where('code', $voucherCode)->first();

            if(!$voucher){
                return redirect()->route('clientCheckout')->withErrors(['voucher'=>"Mã giảm giá không chính xác!"]);


            }else{
                $voucherValue = $voucher->value;
                session()->put('voucherValue', $voucherValue);
